...
public class ShoppingCartActivity extends BaseListActivity {
    ...
    public void requestDatas() {
        if (StaticValues.userModel != null) {
            ...
            HttpMethods.getInstance().cartGoods(new NoProgressSubscriber<List<CarShopModel>>(listener, this));
        } else {
            ...
            if (StaticValues.userModel != null) {
                ...
                SubscriberOnNextListener listener = new SubscriberOnNextListener<HttpResult>() {
                    ...
                    public void onNext(HttpResult httpResult) {
                        T.showShort(ShoppingCartActivity.this, httpResult.message);
                        if (httpResult.code.equals("1")) {
                            requestDatas();
                        }
                    }
                };
                HttpMethods.getInstance().delShoppingCart(new ProgressSubscriber<HttpResult>(listener, this), goodsIds.toString(), priceIds.toString());
            } else {
                realm.beginTransaction();
                for (CarShopModel shopModel : carShopList) {
